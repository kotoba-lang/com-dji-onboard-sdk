(ns dji_onboard_sdk.main
  "Kotodama WASM entrypoint for the Dji_onboard_sdk clean-room actor (L4) — Clojure port.
  ...
  facts (`dji_onboard_sdk.<Entity>/<field>`); `*store*` is the in-memory
  materialization used by the contract test and by the WASM runtime before a
  live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dji_onboard_sdk")
(def tier "L4")
...
(def entity-specs
  [{:entity "Drone"      :plural "drones"      :id-prefix "djionboa_dro"
   ...
   {:entity "Telemetry"  :plural "telemetries" :id-prefix "djionboa_tel"
   ...
   {:entity "Payload"    :plural "payloads"    :id-prefix "djionboa_pay"
    :fields [:droneId :type :weightKg]            :required [:type :weightKg]
    :coerce {:weightKg :float}           :refs {:droneId "Drone"}}
   {:entity "Swarm"      :plural "swarms"      :id-prefix "djionboa_swa"
...
(defn emit-facts
  "EAVT facts for one record: {\"dji_onboard_sdk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dji_onboard_sdk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dji_onboard_sdk-compat/ready)
