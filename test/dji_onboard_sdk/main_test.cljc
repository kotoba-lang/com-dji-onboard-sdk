(ns dji_onboard_sdk.main-test
  "Contract + behavioral test for the dji_onboard_sdk-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "dji_onboard_sdk-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'dji_onboard_sdk.main-test)]
